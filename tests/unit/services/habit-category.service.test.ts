...
import { habitCategoryService } from '../../../src/services/habit-category.service';
...
describe('HabitCategoryService', () => {
  ...
  it('lists categories for user', async () => {
    ...
    const categories = await habitCategoryService.listCategories(USER_ID);
  ...
  it('throws ForbiddenError for non-owner', async () => {
    ...
    await expect(habitCategoryService.getCategoryById(CATEGORY_ID, USER_ID)).rejects.toThrow(
      ForbiddenError
    );
  });
